/* 
 *   $Id: mpio.h,v 1.16 2003/04/28 21:18:38 tds Exp $    
 *
...
/* nonblocking calls currently use MPI_Request, because generalized
...
int PMPI_File_iread(MPI_File fh, void *buf, int count, MPI_Datatype
	                    datatype, MPI_Request *mpi_request); 
...
/* The foll. functions are required, because an MPI_Request object
   is currently used for nonblocking I/O. These functions will go away
   after generalized requests are implemented. */
int PMPIO_Test(MPI_Request *request, int *flag, MPI_Status *status);
int PMPIO_Wait(MPI_Request *request, MPI_Status *status);
MPI_Fint PMPIO_Request_c2f(MPI_Request request);
MPI_Request PMPIO_Request_f2c(MPI_Fint request);
